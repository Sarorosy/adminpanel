...
const TermsOfUse = () => {
    ...
    const fetchTermsOfUse = async () => {
      try {
        const response = await axios.get(
          "https://ryupunch.com/leafly/api/Admin/get_termsofuse",
          {
            headers: {
  ...
  const handleSubmit = async () => {
    try {
        const response = await axios.post(
          "https://ryupunch.com/leafly/api/Admin/update_terms_of_use",
          { content : value },
          {
            headers: {
              Authorization: `Bearer ${user.token}`,
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white p-6 rounded-lg shadow-md mt-6">
      <h2 className="text-2xl font-semibold mb-4 text-gray-700">Terms Of Use</h2>
      <ReactQuill
        value={value}
        onChange={setValue}
        modules={modules}
